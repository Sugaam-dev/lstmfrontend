...
import { Save, Play, RotateCcw, ChevronRight, Loader2 } from 'lucide-react'
...
export default function Configure() {
  ...
  // Pre-fill form once config loads
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <div className="pt-14 flex h-screen">
        ...
        <aside className="w-64 shrink-0 border-r border-border h-full overflow-y-auto pt-8 pb-8 px-4 flex flex-col gap-1">
        ...
        {/* ── Main content ──────────────────────────────── */}
        <main className="flex-1 overflow-y-auto pt-8 pb-24 px-10 max-w-3xl animate-slide-up">
          {/* Section header */}
          <div className="mb-8">
            <p className="font-mono text-[9px] text-accent tracking-[0.25em] mb-1">
              CONFIGURATION → {currentSection.title.toUpperCase()}
            </p>
            <h2 className="font-display text-2xl font-700 text-text">{currentSection.title}</h2>
            <p className="font-body text-sm text-dim mt-1">{currentSection.description}</p>
          </div>

          {/* Fields */}
          <div className="bg-surface border border-border rounded-lg px-6 divide-y divide-border/0">
          ...
          <div className="mt-8 bg-surface border border-border rounded-lg p-5">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-2">
              {Object.entries(form).filter(([k]) => !['id','clerk_user_id','created_at','updated_at'].includes(k)).map(([k, v]) => (
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-surface/90 backdrop-blur border-t border-border px-8 py-4 flex items-center justify-between z-40">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 rounded bg-muted hover:bg-border text-text border border-border font-mono text-xs tracking-wider transition-all disabled:opacity-40"
          >
            {saving
          ...
          <button
            onClick={handleStart}
            disabled={saving || starting}
            className="flex items-center gap-2 px-6 py-2 rounded bg-accent hover:bg-accent/80 text-bg font-mono text-xs font-semibold tracking-wider transition-all shadow-glow disabled:opacity-40"
          >
            {starting
              ? <Loader2 size={13} className="animate-spin" />
              : <Play size={13} />
